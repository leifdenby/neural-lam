...
def test_train_with_mllam_dataset():
    ...
    dataset = GraphWeatherModelDataset(dataset_path=FP_TESTDATA, n_prediction_timesteps=n_prediction_timesteps)
    ...
    assert item["target_states"].shape == (n_prediction_timesteps, n_grid, n_state_features)
    assert item["forcing_windowed"].shape == (n_prediction_timesteps, n_grid, n_forcing_features*(n_input_steps+1))
