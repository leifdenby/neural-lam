...
class MllamDataset(torch.utils.data.Dataset):
    def __init__(self, dataset_path, n_prediction_timesteps):
        ...
        if (
            len(self.ds.time)
            < self.n_input_timesteps + self.n_prediction_timesteps
        ):
            raise ValueError(
...
class GraphWeatherModelDataset(MllamDataset):
    ...
    def __getitem__(self, idx):
        ds_sample = self.ds.isel(
            time=slice(
                idx, idx + self.n_input_timesteps + self.n_prediction_timesteps
            )
        )

        # order the dimensions as the model expects
        for var in self.DATASET_VARIABLES:
            ds_sample[var] = ds_sample[var].transpose(
                *self.DATASET_VARIABLES[var]
            )
        ...
        for i in range(self.n_input_timesteps + 1):
            da_forcing_window = ds_sample.isel(
                time=slice(i, i + self.n_prediction_timesteps)
            ).forcing
