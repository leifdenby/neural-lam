...
def load_graph(graph_dir_path, device="cpu"):
    """
    Backwards-compatible shim for loading graph data.

    Prefer using neural_lam.graph_data.load_graph, which returns a
    GraphEdgesAndFeatures object.
    """
    # Local
    from .graph_data import load_graph as _load_graph

    graph = _load_graph(graph_dir_path=graph_dir_path, device=device)
    return graph.hierarchical, graph.as_dict()
